...
def InTPLMemorySaveMode(modelSpecs):
    if 'TPLMemorySave' not in modelSpecs:
        return False
...
## conduct one-hot encoding of a protein sequence, which consists of a bunch of amino acids.
# Each amino acid is an element in AAs
def SeqOneHotEncoding(sequence):
    seq2int = (np.array(list(map(ord, sequence))) - ord('A')).astype(np.int32)
