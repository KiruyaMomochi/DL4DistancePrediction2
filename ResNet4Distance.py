...
class ResConv1DLayer(object):
	...
	def __init__(self, rng, input, n_in=0, n_out=0, halfWinSize=0, activation=T.nnet.relu, mask=None):
...
class ResConv2DLayer:
	...
	def __init__(self, rng, input, n_in=0, n_out=0, halfWinSize=0, mask=None,
...
class BottleneckBlock:
	...
	def __init__(self, rng, input, n_in, halfWinSize=1, mask=None, n_out=None,
		n_bottleneck=None, activation=T.tanh, dim_inc_method='partial_projection', batchNorm=False):
...
class ResNet:
	def __init__(self, rng, input, n_in, halfWinSize=0,
				 mask=None, n_hiddens=None, n_repeats=None, activation=T.nnet.relu,
				 dim_inc_method='partial_projection', batchNorm=True, version='ResNetv2'):
		"""
		...
		## due to some implementation error, both ResNet2D and ResNet2DV21 use ResBlockV2
		## ResNet2DV23 uses ResBlockV23 and ResNet2DV22 uses ResBlockV22
		ResBlock = ResBlockV2
		if version.endswith('V23'):
			ResBlock = ResBlockV23
		elif version.endswith('V22'):
			ResBlock = ResBlockV22

		blocks = []
		startLayer = ConvLayer(rng, input=input2, n_in=n_in, n_out=n_hiddens[0], halfWinSize=halfWinSize, mask=mask, activation=activation)
		...
		for j in range(n_repeats[0]):
			...
			new_block = ResBlock(rng, input=curr_block.output, n_in=n_hiddens[0], mask=mask, halfWinSize=halfWinSize, activation=activation, dim_inc_method=dim_inc_method, batchNorm=batchNorm)
		...
		for i in range(1, len(n_hiddens)):
			...
			new_block = ResBlock(rng, input=curr_block.output, n_in=n_hiddens[i-1], n_out=n_hiddens[i], mask=mask, halfWinSize=halfWinSize, activation=activation, dim_inc_method=dim_inc_method, batchNorm=batchNorm)
			blocks.append(new_block)
			curr_block = new_block
			...
			for j in range(n_repeats[i]):
				...
				new_block = ResBlock(rng, input=curr_block.output, n_in=n_hiddens[i], mask=mask, halfWinSize=halfWinSize, activation=activation, dim_inc_method=dim_inc_method, batchNorm=batchNorm)
				blocks.append(new_block)
				curr_block = new_block
