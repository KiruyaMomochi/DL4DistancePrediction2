...
class ResNet4DistMatrix:
	def __init__(self, rng, seqInput, matrixInput, mask_seq=None, mask_matrix=None,
				 embedInput=None, modelSpecs = None):
		"""
		...
		if modelSpecs['network'].startswith('DilatedResNet'):
			seqConv = DilatedResNet(rng, input=seqInput, n_in=n_in_seq, n_hiddens=n_hiddens_seq, n_repeats=seq_repeats, halfWinSize=hwsz_seq, dilation=dilation_seq, mask=mask_seq, activation=modelSpecs['activation'], batchNorm=modelSpecs['batchNorm'], version=modelSpecs['network'])
		else:
		...
		if 'OuterCat' in seq2matrixMode and useSequentialFeatures:
			...
			midpointfeature, n_midpoint_out = MidpointFeature(seqConv.output, seqConv.n_out)
		...
		if embedInput is not None:
			...
			embedLayer = MetaEmbeddingLayer(embedInput, modelSpecs['n_in_embed'], n_out_embed)
		...
		if modelSpecs['network'].startswith('ResNet'):
			matrixConv=ResNet(rng, input=input_2d, n_in=n_input2d, n_hiddens=n_hiddens_matrix, n_repeats=matrix_repeats, halfWinSize=hwsz_matrix, mask=mask_matrix, activation=modelSpecs['activation'], batchNorm=modelSpecs['batchNorm'], version=modelSpecs['network'])
		elif modelSpecs['network'].startswith('DilatedResNet'):
			matrixConv=DilatedResNet(rng, input=input_2d, n_in=n_input2d, n_hiddens=n_hiddens_matrix, n_repeats=matrix_repeats, halfWinSize=hwsz_matrix, dilation=dilation_matrix, mask=mask_matrix, activation=modelSpecs['activation'], batchNorm=modelSpecs['batchNorm'], version=modelSpecs['network'])
		else:
		...
		for res in modelSpecs['responses']:
			...
			## output in 2d matrix
			output_2d = predictor.y_pred.reshape( (conv_out.shape[0], conv_out.shape[1], conv_out.shape[2], config.responseValueDims[labelType]) )
			output_2d_prob = predictor.output.reshape( (conv_out.shape[0], conv_out.shape[1], conv_out.shape[2], config.responseProbDims[labelType] ) )
	...
	def loss(self, zList, weightList=None, useMeanOnly=False):
		...
		if weightList is not None and len(weightList)>0:
			for predictor, z, w in zip(self.predictors, zList, weightList):
				...
				## zflag shall have shape (batchSize, valueDims) and wflag shall have shape (batchSize, 1)
	...
	## this function returns a 3*2 matrix where rows 0, 1, and 2 correspond to long-, medium- and short-range accuracy.
	...
	##def TopAccuracyByRange(self, zList, topRatio=0.5):
